...
import re
import weakref
from CARLA.yolo.YOLOv7 import YOLO
...
# Define a global variable to initialize the YOLOv7 model
detector = YOLO("CARLA/weights/yolov7-tiny_256x320.onnx", conf_thres=0.5, iou_thres=0.5)
...
class World(object):
    ...
    def spawn_npcs(self, args):
        self.NPC = NPCClass()
        self.vehicles_list, _ = self.NPC.create_npcs(args)
...
class NPCClass:
    ...
    def create_npcs(self, args):
        ...
        try:
            ...
            if args.nv < number_of_spawn_points:
                random.shuffle(spawn_points)
            elif args.nv > number_of_spawn_points:
                msg = 'requested %d vehicles, but could only find %d spawn points'
                logging.warning(msg, args.nv, number_of_spawn_points)
                args.nv = number_of_spawn_points
            
            # Get vehicles blueprint
            ...
            raise Exception(f"Error spawning vehicles: {e}")
        ...
        try:
            ...
            # Spawn walkers on sidewalks
            batch = []
            walker_speed = []
            for n, location in enumerate(spawn_points):
                bp = random.choice(blueprints)
                if bp.has_attribute('is_invincible'):
                    bp.set_attribute('is_invincible', 'false')
                if bp.has_attribute('speed'):
                    walker_speed.append(bp.get_attribute('speed').recommended_values[1])
                batch.append(SpawnActor(bp, location))
            ...
            for i in range(len(self.walkers_list)):
                batch.append(SpawnActor(blueprints, carla.Transform(), self.walkers_list[i]["id"]))
            ...
            # Combine walkers and controllers id to get the objects from their id
            for i in range(len(self.walkers_list)):
            ...
            for i in range(0, len(self.all_id), 2):
                self.all_actors[i].start()    # start walker
                self.all_actors[i].go_to_location(world.get_random_location_from_navigation())    # set walk to random point
                self.all_actors[i].set_max_speed(float(walker_speed[int(i/2)]))    # max speed

        except Exception as e:
            raise Exception(f"Error spawning walkers: {e}")
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            boxes, scores, class_ids = detector(array)
            array = detector.draw_detections(array)
...
def main():
    ...
    argparser.add_argument(
        ...
        default=50,
        type=float,
        help='NPCs vehicles to spawn (default: 50)')
    argparser.add_argument(
        '-nw',
        default=30,
        type=float,
        help='NPCs walkers to spawn (default: 30)')
